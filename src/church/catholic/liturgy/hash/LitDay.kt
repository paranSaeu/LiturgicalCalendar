...
interface LitDay {
    ...
    val idCat       : IdCat
    val scope       : String
    val litGrade    : LitGrade
    val litColour   : LitColour
    val idName      : String
    ...
    fun encode(): String {
        ...
        sb.append(idCat.cat).append(".")
        sb.append(litGrade.grade).append(".")
        sb.append(litColour.colour).append(".")
    ...
    companion object {
        fun of(id: String): LitDay{

            return decode(id)
        }

        fun validate(idStr: String): Boolean{
            return true
        }

        private fun decode(id: String): LitDay {
            if(!validate(id)) error("Invalid ID : $id")

            val component = id.split(".")

            val idCat       = IdCat.valueOf(component[0])
            val scope       = component[1]
            val litGrade    = LitGrade.valueOf(component[2])
            val litColour   = LitColour(component[3].toInt())
            val idName      = component[4]

            val dayName = {
                ""
            }

            return DbDay(idCat, scope, litGrade, litColour, idName, dayName())
        }
    }
...
enum class IdCat ( var cat: String ) {
    FIXED_FEAST             ("fix"),
    MOVEABLE_FEAST          ("mov");

    companion object {
        fun fromIdStr(id: String): IdCat =
            when(LitDay.validate(id)) {
                true -> fromCatStr(id.take(3))
                else -> error("Invalid Id: $id")
            }

        fun fromCatStr(cat: String): IdCat =
...
enum class LitTemp ( var temp: String ) {
    ...
    companion object {
        fun fromIdStr(id: String): LitTemp =
            when(LitDay.validate(id)) {
                true -> fromCode(id.substring(1, 3))
                else -> error("Invalid Id: $id")
            }
...
value class LitColour( val colour: Int ) {
    ...
    companion object {
        ...
        fun fromIdStr(id: String): LitColour =
            when(LitDay.validate(id)) {
                true -> from(id.split(".")[2].toInt())
                else -> error("Invalid ID : $id")
            }
    }
}